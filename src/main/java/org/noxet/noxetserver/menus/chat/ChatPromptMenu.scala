...
class ChatPromptMenu(message: String, promptedPlayers: Set[Player], callback: ChatPromptMenu.PromptResponse => Unit) extends Listener:
    ...
    private def dispatchPlayerMessage(player: Player, value: String): Unit =
        ...
        if notAnswered.isEmpty then
            stop()
        callback(ChatPromptMenu.PromptResponse(player, value))
