...
class ResetRealmSpawn extends CommandExecutor:
    override def onCommand(commandSender: CommandSender, command: Command, s: String, strings: Array[String]): Boolean =
        if !commandSender.isInstanceOf[Player] then
            ErrorMessage(ErrorMessage.ErrorType.COMMON, "Only players can reset realm spawns.").send(commandSender)
            return true

        val player = commandSender.asInstanceOf[Player]

        if !player.isOp then
            ErrorMessage(ErrorMessage.ErrorType.PERMISSION, "Only operators can reset realm spawns.").send(commandSender)
            return true

        RealmDataManager().setSoftSpawnLocation(getCurrentRealm(player), null)
