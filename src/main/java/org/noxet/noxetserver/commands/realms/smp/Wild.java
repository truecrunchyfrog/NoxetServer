...
public class Wild implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(!(commandSender instanceof Player)) {
            new ErrorMessage(ErrorMessage.ErrorType.COMMON, "Only players can go to the wild.").send(commandSender);
            return true;
        }
        ...
        if(recentlyTeleported.contains(player)) {
            new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You recently got a wilderness teleport. You must wait 2 minutes between requests.").send(player);
            return true;
        }
        ...
        if(realm == null || !realm.doesAllowTeleportationMethods()) {
            new ErrorMessage(ErrorMessage.ErrorType.COMMON, "Wilderness teleportation is not supported here.").send(player);
            return true;
        }

        new Message("Wilderness teleportation commencing. Please wait ...").send(player);
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(NoxetServer.getPlugin(), () -> {
            ...
            do {
                if(attemptsLeft-- == 0) {
                    new ErrorMessage(ErrorMessage.ErrorType.COMMON, "Uh oh. We searched far and wide, but we could not find a suiting place for you.").send(player);
                    break;
                }
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    new BukkitRunnable() {
                        @Override
                        public void run() {
                            new Message("Welcome to the wild!").send(player);
