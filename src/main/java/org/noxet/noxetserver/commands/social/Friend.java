...
public class Friend implements TabExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(!(commandSender instanceof Player)) {
            new ErrorMessage(ErrorMessage.ErrorType.COMMON, "Only players can have friends.").send(commandSender);
            return true;
        }
        ...
        if(strings[0].equalsIgnoreCase("add")) {
            if(strings.length < 2) {
                new ErrorMessage(ErrorMessage.ErrorType.ARGUMENT, "Missing argument: player to befriend.").send(player);
                return true;
            }
            ...
            if(areFriends(player.getUniqueId(), uuidToBefriend)) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You are already friends!").send(player);
                return true;
            }

            if(strings.length > 2 && strings[2].equals("!") && player.isOp()) {
                acceptRequest(uuidToBefriend, player.getUniqueId());
                new SuccessMessage("Forced friendships are the best! You are now friends with " + befriendName + ".").send(player);
                return true;
            }

            if(hasReceivedFriendRequestFrom(uuidToBefriend, player.getUniqueId())) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You have already sent a friend request to them. Please wait for them to respond.").send(player);
                return true;
            }

            if(getFriendList(player.getUniqueId()).size() >= maxFriends) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You have reached your friend limit.").send(player);
                return true;
            }

            if(new PlayerDataManager(player).doesContain(PlayerDataManager.Attribute.BLOCKED_PLAYERS, uuidToBefriend.toString())) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You have blocked this player, and cannot send friend requests to them.").send(player);
                return true;
            }
            ...
            if(
                ...
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You may not send friend requests to this player.").send(player);
                return true;
            }

            if(getFriendList(uuidToBefriend).size() >= maxFriends) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, befriendName + " has reached their friend limit!").send(player);
                return true;
            }
            ...
            if(hasReceivedFriendRequestFrom(player.getUniqueId(), uuidToBefriend)) {
                ...
                new SuccessMessage("You are now friends with " + befriendName + "!").send(player);
                if(playerToBefriend != null)
                    new SuccessMessage(player.getName() + " accepted your friend request.").send(playerToBefriend);

                return true;
            }

            if(getOutgoingFriendRequests(player.getUniqueId()).size() >= maxOutgoingRequests) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, "You have reached your limit on max amount of outgoing friend requests.").send(player);
                return true;
            }

            if(getIncomingFriendRequests(uuidToBefriend).size() >= maxIncomingRequests) {
                new ErrorMessage(ErrorMessage.ErrorType.COMMON, befriendName + " has enough friend requests to take care of, let them be!").send(player);
                return true;
            }

            sendFriendRequest(player.getUniqueId(), uuidToBefriend);

            new Message("§eYou sent a friend request to " + befriendName + ".").send(player);

            if(playerToBefriend != null)
                new Message("§e" + player.getName() + " sent you a friend request!")
        ...
        if(strings[0].equalsIgnoreCase("remove")) {
            if(strings.length < 2) {
                new ErrorMessage(ErrorMessage.ErrorType.ARGUMENT, "Missing argument: player to unfriend.").send(player);
                return true;
            }
            ...
            new SuccessMessage(unfriendName + " is no longer your friend.").send(player);
            if(playerToUnfriend != null)
                new Message("§c" + player.getName() + " removed you as their friend.").send(playerToUnfriend);
        ...
        if(strings[0].equalsIgnoreCase("deny")) {
            if(strings.length < 2) {
                new ErrorMessage(ErrorMessage.ErrorType.ARGUMENT, "Missing argument: player to deny.").send(player);
                return true;
            }
            ...
            new SuccessMessage("Denied the friend request from " + denyName + ".").send(player);
            if(playerToDeny != null)
                new Message("§c" + player.getName() + " denied your friend request.").send(playerToDeny);
        ...
        if(strings[0].equalsIgnoreCase("cancel")) {
            if(strings.length < 2) {
                new ErrorMessage(ErrorMessage.ErrorType.ARGUMENT, "Missing argument: player to cancel request to.").send(player);
                return true;
            }
            ...
            new SuccessMessage("Canceled the friend request to " + cancelName + ".").send(player);
        ...
        if(strings[0].equalsIgnoreCase("toggle-allow-incoming")) {
            ...
            new SuccessMessage((setTo ? "Disabled" : "Enabled") + " incoming friend requests. Other players can " + (setTo ? "no longer" : "now") + " send friend requests to you.").send(player);

            if(setTo && playerDataManager.getListSize(PlayerDataManager.Attribute.INCOMING_FRIEND_REQUESTS) != 0)
                new NoteMessage("You have incoming friend requests already pending. If you want, you can cancel those manually.")
        ...
        if(strings[0].equalsIgnoreCase("toggle-friend-tp")) {
            ...
            new SuccessMessage((setTo ? "Enabled" : "Disabled") + " friend teleportation. Your friends can " + (setTo ? "now" : "no longer") + " /tpa to you without you accepting.").send(player);

            return true;
        }

        new ErrorMessage(ErrorMessage.ErrorType.ARGUMENT, "Sorry, '" + strings[0] + "' is not a valid friend command.").send(player);
