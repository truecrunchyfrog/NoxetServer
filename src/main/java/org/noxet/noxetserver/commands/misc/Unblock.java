...
public class Unblock implements TabExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(!(commandSender instanceof Player)) {
            new NoxetErrorMessage(NoxetErrorMessage.ErrorType.COMMON, "Only players can unblock players.").send(commandSender);
            return true;
        }
        ...
        if(!playerDataManager.doesContain(PlayerDataManager.Attribute.BLOCKED_PLAYERS, uuidToUnblock.toString())) {
            new NoxetErrorMessage(NoxetErrorMessage.ErrorType.COMMON, "You have not blocked " + unblockName + ".").send(player);
            return true;
        }

        playerDataManager.removeFromStringList(PlayerDataManager.Attribute.BLOCKED_PLAYERS, uuidToUnblock.toString()).save();

        new NoxetSuccessMessage(unblockName + " is no longer blocked. They player can now message, friend request and TPA you.").send(player);
