...
import org.noxet.noxetserver.util.{QuickRunnable, TextBeautifier}
...
object RealmManager:
  ...
  enum Realm(
          ...
          ComponentBuilder(
            ...
            .append(
              ComponentBuilder("§2§l■ Thanks, I'll remember this.")
                .event(ClickEvent(ClickEvent.Action.RUN_COMMAND, Events.TemporaryCommand.UnderstandAnarchy.getSlashCommand))
    ...
    def getPlayers: Set[Player] = getWorlds.flatMap(_.getPlayers.asScala).toSet
   ...
   */
  def getRealmFromWorld(world: World): Option[Realm] =
    NoxetServer.ServerWorld.values.find(_.getWorld == world).map(_.getRealm)
  ...
  def migrateToRealm(player: Player, toRealm: Option[Realm]): Unit =
    ...
    QuickRunnable(setPlayerMigrationStatus(player, false))
    ...
    toRealm match
      case Some(r) => // Destination is a realm
        ...
        r.getSpawnLocation match
          ...
          case None => throw Exception(s"realm $r has no valid spawn location")
  ...
  def getMainSpawn: Location =
    RealmDataManager().getSpawnLocation(null).getOrElse(NoxetServer.ServerWorld.HUB.getWorld.getSpawnLocation)

  def getSpawnLocation(player: Player): Location =
    getCurrentRealm(player).map(_.getSpawnLocation).getOrElse(getMainSpawn)
