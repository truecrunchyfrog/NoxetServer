...
class Captcha(player: Player):
  ...
  def init(): Unit =
    ...
    bukkitTasks.addOne(new BukkitRunnable {
    ...
    bukkitTasks.addOne(new BukkitRunnable {
      override def run(): Unit = nextQuestion(0)
    }.runTaskLater(NoxetServer.getPlugin, 20 * delay))


  private def nextQuestion(index: Int): Unit =
    ...
    bukkitTasks.addOne(new BukkitRunnable {
      override def run(): Unit =
        CaptchaSelectionMenu(captchaSounds, index + 1, totalQuestions, captchaSound -> chooseAnswer(captchaSound)).openInventory(player)
  ...
  private def chooseAnswer(answer: CaptchaSound): Unit =
    if correctAnswer.isEmpty then
      Message("§cPlease wait!").send(player)
      return
    ...
    if answer == correctAnswer then
      ...
      if lastQuestion < totalQuestions - 1 then
        for
          i <- 60 to 0 by -20
        do
        ...
        bukkitTasks.add(new BukkitRunnable {
          override def run(): Unit = nextQuestion(lastQuestion + 1)
  ...
  def finish(): Unit =
    ...
    PlayerDataManager(player).set(PlayerDataManager.Attribute.HAS_DONE_CAPTCHA, true).save()
...
object Captcha:
  ...
  private val freezer: PlayerFreezer = PlayerFreezer(20)
  ...
  private def getPlayerCaptcha(player: Player): Captcha = captchaInstances.get(player)

  def isPlayerDoingCaptcha(player: Player): Boolean = captchaInstances.containsKey(player)
  ...
  def stopPlayerCaptcha(player: Player): Unit =
    val captcha = getPlayerCaptcha(player)
    if captcha != null then
      captcha.stop()